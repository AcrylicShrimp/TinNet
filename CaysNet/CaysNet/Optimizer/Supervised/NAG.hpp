...
{
	template<class LossFunc> void NAG::train(std::vector<std::vector<float>> &sInput, std::vector<std::vector<float>> &sOutput, std::size_t nBatchSize, std::size_t nEpoch)
	{
		...
		for (std::size_t nEpochCount{0}; nEpochCount < nEpoch; ++nEpochCount)
		{
			...
			while (nBatchIndex < sInput.size())
			{
				...
				{
					for (std::size_t nBiasIndex{0}, nBiasSize{this->sBiasDelta[nIndex].size()}; nBiasIndex < nBiasSize; ++nBiasIndex)
						this->sBiasDeltaBuffer[nIndex][nBiasIndex] = this->nMomentumTerm * this->sBiasMomentum[nIndex][nBiasIndex] + nFactor * this->sBiasDelta[nIndex][nBiasIndex];

					for (std::size_t nWeightIndex{0}, nWeightSize{this->sWeightDelta[nIndex].size()}; nWeightIndex < nWeightSize; ++nWeightIndex)
						this->sWeightDeltaBuffer[nIndex][nWeightIndex] = this->nMomentumTerm * this->sWeightMomentum[nIndex][nWeightIndex] + nFactor * this->sWeightDelta[nIndex][nWeightIndex];
				}

				for (std::size_t nIndex{0}, nDepth{this->sNN.depth()}; nIndex < nDepth; ++nIndex)
				{
					for (std::size_t nBiasIndex{0}, nBiasSize{this->sBiasDelta[nIndex].size()}; nBiasIndex < nBiasSize; ++nBiasIndex)
						this->sBiasMomentum[nIndex][nBiasIndex] = this->nMomentumTerm * this->sBiasDeltaBuffer[nIndex][nBiasIndex] + nFactor * this->sBiasDelta[nIndex][nBiasIndex];

					for (std::size_t nWeightIndex{0}, nWeightSize{this->sWeightDelta[nIndex].size()}; nWeightIndex < nWeightSize; ++nWeightIndex)
						this->sWeightMomentum[nIndex][nWeightIndex] = this->nMomentumTerm * this->sWeightDeltaBuffer[nIndex][nWeightIndex] + nFactor * this->sWeightDelta[nIndex][nWeightIndex];
				}
