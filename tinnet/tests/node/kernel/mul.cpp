...
TEST_CASE("tinnet::node::kernel::BasicArithmetic mul")
{
	...
	{
		for (std::size_t nIndex{0}; nIndex < nLength; ++nIndex) {
			CHECK(pLeft->gradient().aligned<float>()[nIndex] == Approx(pRight->output().aligned<float>()[nIndex]));
			CHECK(pRight->gradient().aligned<float>()[nIndex] == Approx(pLeft->output().aligned<float>()[nIndex]));
		}
	}
}