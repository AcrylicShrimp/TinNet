...
TEST_CASE("tinnet::node::kernel::BasicArithmetic div")
{
	...
	SECTION("Check forward")
	{
		for (std::size_t nIndex{0}; nIndex < nLength; ++nIndex)
			CHECK(pResult->output().aligned<float>()[nIndex] == Approx(sLeft[nIndex] / (sRight[nIndex] + 1e-5f)));
	}
	...
	SECTION("Check backward")
	{
		for (std::size_t nIndex{0}; nIndex < nLength; ++nIndex) {
			CHECK(
				pLeft->gradient().aligned<float>()[nIndex]
				== Approx(1.f / (pRight->output().aligned<float>()[nIndex] + 1e-5f)));
			CHECK(
				pRight->gradient().aligned<float>()[nIndex]
				== Approx(
					pLeft->output().aligned<float>()[nIndex]
					/ (pRight->output().aligned<float>()[nIndex] * pRight->output().aligned<float>()[nIndex] + 1e-5f)));
		}
	}
}