...
{
	ReduceMax::ReduceMax(Graph *pGraph, const std::string &sName, const std::vector<bool> &sAxis, bool bSqueeze) :
	...
	const Shape &ReduceMax::shape() const
	{
		return this->sShape;
	}

	std::string ReduceMax::type() const
	{
		return ReduceMax::typeName();
	}

	void ReduceMax::notifyShapeUpdated()
	{
	...
	void ReduceMax::forwardPass(Cache sDestination)
	{
	...
	void ReduceMax::backwardPass(Cache sDestination, NodePtr pInput)
	{
