...
{
	Convolution::Convolution(Graph *pGraph, const std::string &sName, NodePtr pSharingNode,
	...
	Shape Convolution::shape() const
	{
		return this->sOutputShape;
	}

	std::string Convolution::type() const
	{
		return Convolution::typeName();
	}

	void Convolution::notifyShapeUpdated()
	{
	...
	void Convolution::initialize(std::function<float()> fInitializer)
	{
